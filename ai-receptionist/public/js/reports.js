(function (global) {
  ...
  function createView(host, opts) {
    ...
    const state = { dashboards: [], contacts: [], fields: [], reportFields: [], currentId: null, charts: [] };
    ...
    async function boot() {
      ...
      try {
        ...
        const [d, c, f] = await Promise.all([dashReq, App.portalApi("/api/contacts"), App.portalApi("/api/fields")]);
        state.dashboards = opts.home ? [d] : d; state.contacts = c; state.fields = f;
    ...
    function openEditor(existing) {
     try {
      ...
      function collect() { const t = $("#w-type").value; const mop = $("#w-mop").value; return { id: w.id, title: $("#w-title").value.trim() || "Untitled", type: t, measure: mop === "count" ? { op: "count" } : { op: mop, field: $("#w-mfield").value }, groupBy: t === "kpi" ? [] : groupEd.getDims(), series: (t === "stacked" || t === "heatmap") ? seriesEd.getDims() : [], filters: w.filters, cw: w.cw, ch: w.ch }; }
