...
(function (global) {
  ...
  function openEditor(host, drip) {
    ...
    const canvas = el("div", "drip-canvas"); canvas.style.cssText = "position:relative;width:100%;height:660px;overflow:hidden;border:1px solid var(--line,#e5e7eb);border-radius:10px;background:#fafafa;background-image:radial-gradient(rgba(0,0,0,.07) 1px, transparent 1px);background-size:22px 22px;cursor:grab";
    ...
    PALETTE_GROUPS.forEach((grp) => {
      ...
      grp.types.forEach((type) => {
        ...
        item.style.cssText = "display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--line,#e5e7eb);border-left:4px solid " + m.accent + ";border-radius:8px;cursor:grab;margin-bottom:6px;background:" + m.bg + ";font-size:13px";
    ...
    function renderNode(node) {
      ...
      if (m.trigger) {
        const d = TRIG_R * 2;
        card.style.cssText = "position:absolute;left:" + node.x + "px;top:" + node.y + "px;width:" + d + "px;height:" + d + "px;border-radius:50%;background:" + m.bg + ";border:2px solid " + (err ? "#ef4444" : m.accent) + ";box-shadow:0 1px 4px rgba(0,0,0,.1);display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;cursor:grab;user-select:none;padding:6px" + (selected ? ";outline:2px solid " + m.accent + ";outline-offset:2px" : "");
        ...
        card.style.cssText = "position:absolute;left:" + node.x + "px;top:" + node.y + "px;width:" + NODE_W + "px;min-height:" + NODE_H + "px;background:" + m.bg + ";border:1px solid " + (err ? "#ef4444" : m.accent) + ";border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,.1);padding:9px 12px;cursor:grab;user-select:none" + (selected ? ";outline:2px solid " + m.accent + ";outline-offset:2px" : "");
