...
Run started **2026-07-30 14:24 UTC** — mode `gate`.
...
| --- | --- | --- |
| Passed | 47 | every assertion in the suite held |
...
| **Total recorded** | **49** | of 49 suites |

Measured time across the recorded suites: **484.6s**. Per-suite durations are in the table below, green ones included, so the cost of the serial database lane can be judged from real numbers rather than a guess.

## Every suite

| suite | lane | result | took | first failing assertion |
| --- | --- | --- | --- | --- |
| `selfTest_devToolsTabs` | server | **FAILED** | 16.9s | Tools renders a strip of exactly one tab labelled "Demo Data" () |
| `selfTest_tenantIdentity` | server | **FAILED** | 53.7s | PICKER OFFERS FIELDS THE CARD IGNORES: name, demo, status, created, ai, calls, contacts, users, actions — add them to buildCard or remove them from the picker |
...
| `selfTest_allThemeContrast` | scanner | passed | 300ms |  |
| `selfTest_auditViewer` | database | passed | 440ms |  |
| `selfTest_bellOrganic` | server | passed | 14.2s |  |
| `selfTest_createUi2` | server | passed | 7.2s |  |
| `selfTest_customerComms` | database | passed | 6.5s |  |
| `selfTest_demoSeeder` | server | passed | 31.1s |  |
| `selfTest_demoTenantSafety` | server | passed | 60.3s |  |
| `selfTest_demoTooling` | server | passed | 18.0s |  |
| `selfTest_designRatchet` | scanner | passed | 333ms |  |
| `selfTest_devToolsShell` | database | passed | 385ms |  |
| `selfTest_domSmoke` | server | passed | 13.7s |  |
| `selfTest_estimates` | database | passed | 3.3s |  |
| `selfTest_fileStorage` | database | passed | 33.9s |  |
| `selfTest_fsPunchlist1` | database | passed | 3.0s |  |
| `selfTest_globalSearchA` | server | passed | 8.8s |  |
| `selfTest_globalSearchB` | server | passed | 7.7s |  |
| `selfTest_hubPolish` | server | passed | 10.6s |  |
| `selfTest_hubPolish3` | server | passed | 7.1s |  |
...
| `selfTest_learningCenter3` | database | passed | 626ms |  |
| `selfTest_linkConventions` | database | passed | 2.8s |  |
| `selfTest_listpageIntegrity` | database | passed | 3.5s |  |
...
| `selfTest_rmContentPack` | server | passed | 8.0s |  |
| `selfTest_rmTemplate1` | server | passed | 9.1s |  |
| `selfTest_routeAwareness` | server | passed | 4.8s |  |
| `selfTest_rowAnatomy` | server | passed | 7.3s |  |
...
| `selfTest_settingsSweep` | server | passed | 10.2s |  |
| `selfTest_suggestions1` | server | passed | 11.1s |  |
| `selfTest_tablePersistence` | server | passed | 6.3s |  |
| `selfTest_tenantsTableUi` | scanner | passed | 270ms |  |
| `selfTest_tenantTemplates1` | server | passed | 6.2s |  |
