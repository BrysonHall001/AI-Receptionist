...
Run started **2026-07-30 14:57 UTC** — mode `gate`.
...
| --- | --- | --- |
| Passed | 48 | every assertion in the suite held |
...
Measured time across the recorded suites: **488.4s**. Per-suite durations are in the table below, green ones included, so the cost of the serial database lane can be judged from real numbers rather than a guess.

## Every suite

| suite | lane | result | took | first failing assertion |
| --- | --- | --- | --- | --- |
| `selfTest_devToolsTabs` | server | **FAILED** | 17.3s | Tools renders a strip of exactly one tab labelled "Demo Data" () |
| `selfTest_tenantIdentity` | server | **FAILED** | 54.5s | PICKER OFFERS FIELDS THE CARD IGNORES: name, demo, status, created, ai, calls, contacts, users, actions — add them to buildCard or remove them from the picker |
| `selfTest_adaptationCounters` | server | passed | 13.5s |  |
| `selfTest_aiIntake` | database | passed | 3.8s |  |
| `selfTest_aiSchedulingTarget` | database | passed | 4.2s |  |
| `selfTest_allThemeContrast` | scanner | passed | 299ms |  |
...
| `selfTest_demoSeeder` | server | passed | 26.4s |  |
| `selfTest_demoTenantSafety` | server | passed | 59.8s |  |
| `selfTest_demoTooling` | server | passed | 18.4s |  |
| `selfTest_designRatchet` | scanner | passed | 345ms |  |
| `selfTest_devToolsShell` | database | passed | 337ms |  |
| `selfTest_domSmoke` | server | passed | 13.8s |  |
...
| `selfTest_globalSearchA` | server | passed | 9.5s |  |
...
| `selfTest_learningCenter3` | database | passed | 571ms |  |
| `selfTest_linkConventions` | database | passed | 2.8s |  |
| `selfTest_listpageIntegrity` | database | passed | 3.3s |  |
...
| `selfTest_settingsSweep` | server | passed | 10.8s |  |
| `selfTest_suggestions1` | server | passed | 11.6s |  |
| `selfTest_tablePersistence` | server | passed | 5.9s |  |
| `selfTest_tenantsTableUi` | scanner | passed | 286ms |  |
| `selfTest_tenantTemplates1` | server | passed | 5.7s |  |
| `selfTest_tenantTemplates2` | server | passed | 7.9s |  |
| `selfTest_transcriptInsights` | server | passed | 7.3s |  |
