...
Run started **2026-08-03 23:02 UTC** — mode `gate`.
...
Measured time across the recorded suites: **182.7s**. Per-suite durations are in the table below, green ones included, so the cost of the serial database lane can be judged from real numbers rather than a guess.

## Every suite

| suite | lane | result | took | first failing assertion |
| --- | --- | --- | --- | --- |
| `selfTest_customerComms` | database | **FAILED** | 2.3s | PrismaClientInitializationError: |
| `selfTest_estimates` | database | **FAILED** | 2.2s | PrismaClientInitializationError: |
| `selfTest_fileStorage` | database | **FAILED** | 2.3s | PrismaClientInitializationError: |
| `selfTest_recurringWork` | database | **FAILED** | 2.6s | PrismaClientInitializationError: |
| `selfTest_runnerHarness` | database | **FAILED** | 5.2s | it names 68 suites (47 inherited from the hand-maintained block, plus the runner harness, selfTest_devToolsTabs and selfTest_widgetChrome) |
| `selfTest_schedulingCalendar` | database | **FAILED** | 2.2s | PrismaClientInitializationError: |
| `selfTest_workOrders1` | database | **FAILED** | 2.7s | PrismaClientInitializationError: |
| `selfTest_adaptationCounters` | server | not run | 4.1s | could not reach the database |
| `selfTest_aiIntake` | database | not run | 2.6s | could not reach the database |
| `selfTest_aiReceptionistTemplate` | database | not run | 1.3s | could not reach the database |
| `selfTest_aiSchedulingTarget` | database | not run | 2.5s | could not reach the database |
| `selfTest_bellOrganic` | server | not run | 3.6s | could not reach the database |
| `selfTest_builderDashboards` | database | not run | 1.7s | could not reach the database |
| `selfTest_builderModulesFields` | database | not run | 1.8s | could not reach the database |
| `selfTest_builderRework` | database | not run | 405ms | could not reach the database |
| `selfTest_builderViewsPipelines` | database | not run | 1.8s | could not reach the database |
| `selfTest_createUi2` | server | not run | 3.6s | could not reach the database |
| `selfTest_demoSeeder` | server | not run | 3.3s | could not reach the database |
| `selfTest_demoTenantSafety` | server | not run | 3.8s | could not reach the database |
| `selfTest_demoTooling` | server | not run | 3.9s | could not reach the database |
| `selfTest_devToolsTabs` | server | not run | 3.9s | could not reach the database |
| `selfTest_domSmoke` | server | not run | 3.6s | could not reach the database |
| `selfTest_foodServiceTemplate` | database | not run | 1.3s | could not reach the database |
| `selfTest_fsPunchlist1` | database | not run | 2.4s | could not reach the database |
| `selfTest_fullscreenPresence` | database | not run | 1.5s | could not reach the database |
| `selfTest_globalSearchA` | server | not run | 4.0s | could not reach the database |
| `selfTest_globalSearchB` | server | not run | 3.9s | could not reach the database |
| `selfTest_hubPolish` | server | not run | 3.9s | could not reach the database |
| `selfTest_hubPolish3` | server | not run | 3.9s | could not reach the database |
| `selfTest_hubUiConsistency` | server | not run | 4.1s | could not reach the database |
| `selfTest_iconLibrary` | database | not run | 1.8s | could not reach the database |
| `selfTest_lcFieldServices` | server | not run | 4.1s | could not reach the database |
| `selfTest_lcRecruitment` | server | not run | 3.8s | could not reach the database |
| `selfTest_linkConventions` | database | not run | 2.1s | could not reach the database |
| `selfTest_listpageIntegrity` | database | not run | 2.5s | could not reach the database |
| `selfTest_mfa` | server | not run | 3.2s | could not reach the database |
| `selfTest_multiVisitCardFix` | server | not run | 4.0s | could not reach the database |
| `selfTest_notifications1` | server | not run | 3.6s | could not reach the database |
| `selfTest_notifUiFit` | server | not run | 4.2s | could not reach the database |
| `selfTest_perModulePerms` | server | not run | 3.1s | could not reach the database |
| `selfTest_presence` | database | not run | 2.8s | could not reach the database |
| `selfTest_priceBook` | database | not run | 2.3s | could not reach the database |
| `selfTest_rmContentPack` | server | not run | 3.7s | could not reach the database |
| `selfTest_rmTemplate1` | server | not run | 3.9s | could not reach the database |
| `selfTest_routeAwareness` | server | not run | 3.8s | could not reach the database |
| `selfTest_rowAnatomy` | server | not run | 4.1s | could not reach the database |
| `selfTest_servicePlans` | server | not run | 3.6s | could not reach the database |
| `selfTest_settingsSweep` | server | not run | 4.1s | could not reach the database |
| `selfTest_ssoSignIn` | database | not run | 475ms | could not reach the database |
...
| `selfTest_tenantIdentity` | server | not run | 3.5s | could not reach the database |
| `selfTest_tenantTemplates1` | server | not run | 3.9s | could not reach the database |
| `selfTest_tenantTemplates2` | server | not run | 3.9s | could not reach the database |
| `selfTest_transcriptInsights` | server | not run | 3.9s | could not reach the database |
| `selfTest_allThemeContrast` | scanner | passed | 406ms |  |
| `selfTest_appShell` | scanner | passed | 1.6s |  |
| `selfTest_auditViewer` | database | passed | 483ms |  |
| `selfTest_designRatchet` | scanner | passed | 322ms |  |
| `selfTest_devToolsShell` | database | passed | 320ms |  |
| `selfTest_helpTips` | scanner | passed | 1.7s |  |
| `selfTest_holidayMark` | scanner | passed | 1.7s |  |
| `selfTest_learningCenter3` | database | passed | 422ms |  |
| `selfTest_motionPolish` | scanner | passed | 1.4s |  |
| `selfTest_permissionsRegroup` | scanner | passed | 372ms |  |
| `selfTest_suiteWaits` | scanner | passed | 295ms |  |
| `selfTest_tenantsTableUi` | scanner | passed | 306ms |  |
