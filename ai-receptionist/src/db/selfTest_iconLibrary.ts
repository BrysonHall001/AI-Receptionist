...
async function main() {
  ...
  const pick = adminSrc.slice(adminSrc.indexOf("function buildIconPicker(host)"), adminSrc.indexOf("function paintIconSelection"));
  check(pick.indexOf('rb.type = "radio"') !== -1,
    "each choice is a real radio input \u2014 tab order and arrow-key walking come from the element, not from re-implementation");
  const css = readFileSync(resolvePath(R, "public", "styles.css"), "utf8");
  const radioRule = css.slice(css.indexOf(".tb-iconradio {"), css.indexOf("}", css.indexOf(".tb-iconradio {")));
  check(radioRule.indexOf("display: none") === -1 && radioRule.indexOf("opacity: 0") !== -1,
    "\u2026hidden with opacity rather than display:none, so it keeps its focus ring and its name");
  check(css.indexOf(".tb-iconopt:focus-within") !== -1, "\u2026and the option shows a visible focus outline when tabbed to");
  check(pick.indexOf("tb-iconname") !== -1 && pick.indexOf("ic.name") !== -1, "\u2026and every glyph is labelled with its name");
