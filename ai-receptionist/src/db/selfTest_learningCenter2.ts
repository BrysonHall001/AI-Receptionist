...
const bespoke = [...used].filter((c) => !/^(scene-|lstep|fun-seg)/.test(c) && !["card", "widget-card", "widget-grid", "kpi", "kpi-value", "kpi-label", "pill", "success", "skipped", "btn", "btn-primary", "btn-ghost", "btn-sm", "btn-block", "nav-item", "active", "input", "field-label", "eyebrow", "icon-btn", "kanban-col", "kanban-col-head", "kanban-cards", "kanban-card", "seg-btn", "seg-on", "chip", "cell-muted", "page-actions", "map-grid", "rule-editor", "rule-field", "rule-op", "rule-val", "cal-head", "cal-dayhead", "tab", "tab-conv", "related-pane", "link-facts", "link-facts-date", "cal-reshead", "cal-resname", "cal-tray", "cal-tray-head", "cal-tray-title", "cal-tray-count", "tpl-cat", "tpl-cat-count"].includes(c)); // LC-3: fidelity scenes use the REAL page classes (+ lc-field-services: the Related-tabs surface's real classes)
