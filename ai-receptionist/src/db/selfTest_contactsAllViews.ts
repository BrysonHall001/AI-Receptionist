...
async function main() {
  ...
  check(/name: "Board", available: boardAvailable/.test(bv) && /name: "Calendar", available: calAvailable/.test(bv) && /name: "Map", available: mapAvailable/.test(bv) && /name: "Gallery", available: galAvailable/.test(bv), "all four tiles render with their standard availability rules"); // boardAvailable since FS Punch List 1 (pipeline OR record statuses)
