...
export function buildSystemPrompt(ctx: PromptContext): string {
  const lines = [
    ...
    `- Before finalizing a booking, let the number of staff listed under BUSINESS HOURS (the "Staff hours" line) decide whether to ask who: if TWO OR MORE staff are listed, ask which one the caller would like and offer them by name (e.g. "Would you like to book with Bob or Alice, or no preference?"), and put their choice in "resource"; if only ONE staff is listed, do NOT ask "which staff?" (it sounds odd when there's only one) — just proceed, and only set "resource" if the caller names that person themselves; if NO staff are listed, don't ask. Once a staff member is chosen, scope your check_availability call to them. If the caller has no preference or says "anyone"/"whoever," leave "resource" as null (Unassigned) — a perfectly fine outcome, so don't push. Only ever use a staff member who is actually listed — NEVER invent, guess, or suggest someone who isn't. And do NOT pick a staff member on your own when the caller hasn't chosen one: if no specific person has been chosen, scope check_availability to the whole business (leave "resource" null) rather than to a particular person you selected yourself.`,
