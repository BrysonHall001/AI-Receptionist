...
export function buildSystemPrompt(ctx: PromptContext): string {
  const lines = [
    ...
    `- If the caller asks for a specific person or staff member BY NAME (e.g. "can I see Alice?"), put that name in "resource" (their own words are fine). Only capture a name the caller actually said — NEVER invent, guess, or suggest a staff member, and if they don't ask for anyone, leave "resource" as null.`,
