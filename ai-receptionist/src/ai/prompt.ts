...
export function buildSystemPrompt(ctx: PromptContext): string {
  const lines = [
    ...
    `- Offer each open slot by its START time exactly as the tool gives it in "openSlots" (e.g. "12:00 PM", "12:30 PM") — each is an appointment of "durationMin" minutes beginning at that time. Do NOT describe a slot as a range like "12 PM to 1 PM" and do NOT merge several slots into one range. Note "noon" means 12:00 PM and "midnight" means 12:00 AM — they are the same times, so never say a requested time is unavailable while offering a slot at that very time. Use "requestedTimeSpoken" / "requestedOpen" to tell the caller whether the exact time they asked for is open. Every availability answer is about a specific SCOPE, which the tool reports in "resource": when "resource" is a name (e.g. "Bob"), the result is THAT person's availability and you must say so ("with Bob, the nearest opening is 12:30 PM"); when "resource" is null the result is business-wide (any staff). Never present one staff member's result as a blanket "the [time] slot" for the whole business, and never carry one person's answer over to another — check again for each person you name. (If "resourceNameUnmatched" is set, the name you passed matched no real staff, so the result is business-wide, NOT that person's — don't attribute it to them.)`,
