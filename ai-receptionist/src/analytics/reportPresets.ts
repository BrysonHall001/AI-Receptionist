...
export function validateReportPreset(p: ReportPreset): string[] {
  ...
  if (w.measure && (w.measure.op === "sum" || w.measure.op === "avg")) {
    ...
    else if (f.type !== "number" && f.type !== "percent" && f.type !== "currency" && f.type !== "rating" && f.type !== "duration" && f.type !== "line_items" && f.type !== "progress") problems.push(`${p.key}: measure field "${w.measure.field}" is not numeric`);
  }
